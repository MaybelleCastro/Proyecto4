...
# 4.1 Modulación 16-QAM (50%). Realice una simulación del sistema de comunicaciones como en la sección 3.2.,pero utilizando
# una modulación 16-QAM en lugar de una modulación BPSK. Deben mostrarse las imágenes enviadas y recuperadas y las
# formas de onda.
import numpy as np
import matplotlib.pyplot as plt
import time
...
def modulador(bits, fc, mpp):
    ...
    # 1. Parámetros de la 'señal' de información (bits)
    N = len(bits) # Cantidad de bits
    #N = len(len(bits)) # Cantidad de bits
    ...
    for i, bit in enumerate(bits):
                                                           # A1
        if (bit == 0) and (x%2 != 0):                      # Numeros pares
        ...
        if (bit == 1) and (x%2 != 0):
            ...
            x += 1
                                                          ...
                                                          # A2
        if (bit == 1) and (x%2 == 0):                     # Numeros impares
